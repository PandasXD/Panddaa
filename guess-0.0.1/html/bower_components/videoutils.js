    ...
    function draw(canvas, data, width, height, color){
        ...
        if (color==13) {
            for (var y=0;y<height;y+=1)
            {
                ...
                {
                    imageData.data[(y*width+x)*4+0]=data[(y*width+width-1-x)*3+2];
                    imageData.data[(y*width+x)*4+1]=data[(y*width+width-1-x)*3+1];
                    imageData.data[(y*width+x)*4+2]=data[(y*width+width-1-x)*3+0];
